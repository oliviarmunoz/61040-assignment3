...
exports.DayPlanner = void 0;
class DayPlanner {
    constructor() {
        this.activities = [];
        this.assignments = [];
    }
    addActivity(title, duration) {
        const activity = {
            title,
            duration
        };
    ...
    assignActivity(activity, startTime) {
     ...
     */
    isAssigned(activity) {
        return this.assignments.some((assignment) => assignment.activity === activity);
    }
    /**
     * Create the prompt for Gemini with hardwired preferences
     */
    createAssignmentPrompt(activities, existingAssignments) {
...
Return ONLY the JSON object, no additional text.`;
     ...
     */
    parseAndApplyAssignments(responseText, unassignedActivities) {
        try {
            ...
            for (const rawAssignment of response.assignments) {
                ...
                const { title, startTime } = rawAssignment;
    ...
    displaySchedule() {
        ...
        for (let slot = 0; slot < 48; slot++) {
            ...
            if (activities.length > 0) {
                ...
                const isActivityStart = activities.some(activity => this.assignments.find(a => a.activity === activity)?.startTime === slot);
                if (isActivityStart) {
...
//# sourceMappingURL=dayplanner.js.map